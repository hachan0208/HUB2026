...
export function getVal(obj: any, searchKeys: string[]): any {
  ...
  const normalizedKeys = searchKeys.map(k => k.trim().toLowerCase());
  for (const k in obj) {
    if (normalizedKeys.includes(k.trim().toLowerCase())) return obj[k];
  }
  return undefined;
}
...
export function formatVNThousands(num: number): string {
  ...
  if (decPart > 0) {
    ...
    // Replace dot with comma for VN standard
    const dec = decPart.toFixed(2).slice(1).replace(".", ",").replace(/0+$/, "");
    if (dec === ",") return (isNeg ? "-" : "") + intStr;
...
export async function autoMapColumns(
  ...
  try {
    ...
    const wb = XLSX.read(buf, { sheetRows: 50 });
