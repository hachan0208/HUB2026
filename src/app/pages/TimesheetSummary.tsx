...
export default function TimesheetSummaryPage({ onSwitchToFinal }: { onSwitchToFinal?: () => void }) {
  ...
  const handleUploadFile = async (rowId: string, file: File) => {
    ...
    try {
      ...
      if (foundData) {
        ...
        throw new Error("File trống hoặc không tìm thấy dòng Header (cần có cột No/Stt)");
      }
    } catch (err: any) {
      console.error(`Error reading file ${file.name}:`, err);
      handleUpdateRow(rowId, "status", "error");
      toast?.error(`Lỗi đọc file ${file.name}: ${err?.message || "Unknown error"}`);
    }
  };
  ...
  const computedData = useMemo(() => {
    ...
    rosterData.forEach((t) => {
      ...
      const center =
        centerInfo?.l07 || rowInfo?.l07 || mapL07(rawCenterCol) || rawCenterCol || "UNKNOWN";
      const maAE =
